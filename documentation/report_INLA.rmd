...
w.pm <- I1nb$summary.random$w$mean
wproj <- inla.mesh.projector(mesh1a, dims=c(xres,yres), xlim = xl, ylim = yl) 
...
```{r, eval=F, echo=T, fig.width=8, fig.height=4, dpi=600}
rho <- I1nb$summary.hy[4,"mean"]
plot(I1nb$marginals.hyper[[4]],xlab= expression(rho),ylab="Density", type="l")
abline(v=rho, col=2)
...
## Simlulate data by sampling from posterior (from I2nb)
