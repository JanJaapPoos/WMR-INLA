...
A2 <- inla.spde.make.A(mesh = mesh1a, loc = Loc,
...
w.st.y <- inla.spde.make.index('w', 
                             n.spde = spde$n.spde,
                             n.group = length(startyr:endyr))
w.st.q <- inla.spde.make.index('w', 
                             n.spde = spde$n.spde,
                             n.group = 2)
...
The model formula also needs to be updated.
```{r, eval=T, echo=T}
fsp <- parse(text=c("y ~ -1 + Intercept + ",paste(c(names(X)," f(w.y, model = spde, group = w.group, control.group = list(model = 'ar1'))"," f(w.q, model = spde, group = w.group, control.group = list(model = 'seasonal'))",' f(Depth,model="rw2")'), collapse =" + ")))
```
...
```{r, eval=T, echo=T}
INLA:::inla.dynload.workaround() 
I6nb <- inla(eval(fsp), family = "nbinomial", data=inla.stack.data(Stack1),
           control.compute = list(dic = TRUE, waic = TRUE, config=TRUE, openmp.strategy="large"),
           control.predictor = list(A = inla.stack.A(Stack1)))
