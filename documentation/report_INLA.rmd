...
The value of any point is created by projecting this point onto a plane formed by the triangle it is positioned in. The exact value of the point is the weighted average of three weights associated with each vertex. Fr more information and a worked out example see also http://www.flutterbys.com.au/stats/tut/tut12.13.html#h2_7. 
...
## Making the SPDE 
INLA uses stochastic partial differential equations (SPDE). A full tutorial on SPDEs can be found at http://inla.r-inla-download.org/r-inla.org/tutorials/spde/spde-tutorial.pdf. Making the SPDE for a 2 dimensional mesh with matern correlation is done using inla.spde2.matern(). This takes the mesh as argument. After the SPDE object is created, we also need to make a list of named index vectors for an SPDE model. This is done inla.spde.make.index().
...
The stack allows INLA to build models with complex linear predictors. Here have a SPDE model combined with covariate fixed effects and an intercept at _n_ hauls.
...
Rather than using time as a measure of effort, we can use the surface of the trawls (swept area) as a response variable. That will allow us to make predictions on the number of individuals caught per unit area. Because of the log link in the model, the surfaces are log transformed (and named "lsurface"). Remember that the units of surface were km2. The parameter for the linear predictor is expected to be 1: in that case a doubling of the surface results in a doubling of the counts.  
...
The depth smoother can be plotted again, now for the Negative Binomial model with spatial-temporal correlation.
```{r, eval=T, echo=T, dpi=600}
#plot depth smoother
...
```
The posterior mean of the temporal correlation parameter is `rho[2]`.

Below, we plot the characteristics of the spatial (Matern) correlation.
```{r, eval=T, echo=T}
...
We need a new A object that contains not only the spatial correlation, but also the temporal correlation. This is done by creating a mesh for the time, where there is an observation for each year. This temporal mesh is used when creating the spde. 
```{r, eval=TRUE, echo=TRUE}
# 2. Define the weighting factors a_ik (also called the projector matrix).
endyr <- max(cpue_subset$Year)
y.mesh <- inla.mesh.1d(loc = startyr:endyr)
