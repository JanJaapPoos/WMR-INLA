...
Now that we have the tow possible boundaries to use for the mesh, we can generate a mesh with each. The generation of the mesh is done using inla.mesh.2d(). That function takes several arguments, incuding "cutoff" and "max.edge". The max.edge argument specifies the largest allowable edge length for the for the triangles in the mesh. Hence, these arguments specify how fine the final mesh will be. Finer meshes will be able to capture smaller scale spatial correlations, but require more computing time in the inla model.
...
I1zinb <- inla(eval(fsp), family = "zeroinflatednbinomial1", data=inla.stack.data(Stack1),
           ...
           control.predictor = list(A = inla.stack.A(Stack1)))
```
Once the INLA models are run, a summary can be printed for the models. This summary contains much of the relevant information for the models. The summary also  contains the DIC and WAIC criterions. DIC and WAIC work just like AIC in that the model with the lowest WAIC should be selected.
...
The range is the distance at which the spatial correlation is approxmately 0.1. Below we plot the correlation as a function of distance, with a hortizontal line where the correlation is 0.1.
...
plot(x = d.vec, y = Cor.M, 
     ...
     ylab = "Correlation", yaxs="i", las=1, ylim=c(-0.001,1))
abline(h = 0.1, lty = 2)
```
...
plot(mnwt~Year, data=annualmnwt, type="b", ylab="Mean weight (kg)", ylim=c(0,max(mnwt)),las=1)
```
