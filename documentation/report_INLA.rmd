...
```{r, eval=T, echo=T}
fsp <- parse(text=c("y ~ -1 + Intercept + ",paste(c(names(X)," f(w.y, model = spde, group = w.group, control.group = list(model = 'ar1'))"," f(w.q, model = spde, group = w.group, control.group = list(model = 'exchangeable'))",' f(Depth,model="rw2")'), collapse =" + ")))
```
...
for (i in unique(w.st$w.group)){
    w.pm1km2 <- inla.mesh.project(wproj,w.pm[w.st.y$w.group==i])
    grid[grid$year==years[i],]$zm <- as.vector(w.pm1km2)  
}
