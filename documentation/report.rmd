...
```
Simple way of plotting resulting depth model (at observations)

```{r, eval=T, echo=T, dpi=600}
plot(cpue_subset[!is.na(cpue_subset$ShootLon),]$ShootLon,
     cpue_subset[!is.na(cpue_subset$ShootLon),]$ShootLat,
     pch=20, col=rev(tim.colors(15, alpha = 1))[cut(depthmodel$fitted.values,20)], 
...
```{r, eval=TRUE, echo=TRUE, dpi=1200}
par(mfrow=c(1,1))
ConvHull <- inla.nonconvex.hull(Loc, convex=-0.02, resolution=90)
mesh1a   <- inla.mesh.2d(boundary = ConvHull,  max.edge=c(30))
plot(mesh1a)
