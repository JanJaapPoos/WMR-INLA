...
BTS$surface <- BTS$Distance * BTS$BeamWidth
...
IBTSHH[IBTSHH$Netopening == -9,]$Netopening <- NA

IBTSHH$surface <- IBTSHH$Distance * IBTSHH$Netopening
```

Select only relevant variables, and remove all hauls longer than 90 minutes
```{r, eval=T, echo=T}
IBTSHH <- IBTSHH[names(IBTSHH) %in% c("haulCode", "surface", "HaulDur","Doorspread","Ditance","Netopening","Warplngt")]
...
summary(lm(surface~HaulDur, data=IBTSHH))
```


Merge haulinfo to full cpue set
```{r, eval=T, echo=T}
IBTS <- merge(IBTS, IBTSHH, all.x=T, all.y=F)
```
