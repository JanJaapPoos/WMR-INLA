...
Given that the depth for a given location does not change much over the years, we use a generalized additive model to model a depth map, and to predict depths for those hauls where depth is missing. 

```{r, eval=T, echo=T}
depthmodel <- gam(Depth~te( ShootLong,ShootLat,k=17), data=cpue_subset)
summary(depthmodel)
gam.check(depthmodel)
plot(depthmodel, pers=T)
