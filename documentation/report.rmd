...
We still have a lot of data (`r nrow(full_cpue)`), Because these are just here as example, we will subset the data (here we only include data since 2010) 
```{r, eval=TRUE, echo=TRUE}
...
The INLA code does not like special characters in some of the variable names, like the hyphen in "NS-IBTS". Therefore we rename the survey to NSIBTS. We also need to convert all fixed effects that are factors in the INLA model.   
```{r, eval=TRUE, echo=TRUE}
...
For our spatial correlation we will need an isomorphic coordinate system. Thereofre we transform the latitudes and longitudes to UTM coordinates. these UTM coordinates are given in meters, so we divide them by 1000 to get kilometers. These UTM coordinates combined into a Loc (location) dataset.  
```{r, eval=TRUE, echo=TRUE}
...
wproj <- inla.mesh.projector(mesh1, xlim = range(Loc[,1]), ylim = range(Loc[,2])) 
...
Next we inspect the length range in the data and make a number of "knots" along the length range that we will later use for our analysis 

```{r, eval=T, echo=T}
  sort(unique(cpue_subset$LngtClass))
  Knots <- seq(from = 10, to = 670, by = 100)
  Knots 
```
...
```{r, eval=T, echo=T}
  ...
  # One-dimensional mesh for time (length class) See the time series module
  mesh.t <- inla.mesh.1d(loc = Knots)
  mesh.t$loc
  mesh.t$n   #We have 7 random fields
```

Now that we have a 1 dimensional mesh for the lengths, we use it  to construct a observation/prediction weight matrix ("projetor matrix") based on the spatial mesh that we already created earlier (mesh1) and our new mesh for the lengths. The lengths are used in the "group model". The new projector matrix is names A2 to distinguish it from the projector matrix of the previous model.

```{r, eval=T, echo=T}
  
  # 2. Define the weighting factors a_ik (also called the projector matrix).
  NGroups <- length(Knots) # = 4  
  A2      <- inla.spde.make.A(mesh  = mesh1, 
                            loc   = Loc,
...
# 5. Make a stack. In this process we tell INLA at which points on the mesh we sampled the response variable and the covariates. 
#model matrix
Xmatrix <- model.matrix(~  fYear + fSurvey +  HaulDur,  data = cpe_subset)

#this is the easier one and works
head(Xmatrix)
X <- as.data.frame(Xmatrix[,-1])
names(X) <- c(gsub("[:]",".",names(X)))
head(X)
...
Stack4 <- inla.stack(
  tag  = "Fit",
  data = list(y = ple_subset_Q$NoPerHaul),    
  A    = list(1,1, A3),                      
  effects = list(  
    Intercept = rep(1, N),       
