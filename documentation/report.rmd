...
```{r, eval=TRUE, echo=TRUE}
# there are some negative  distances. We make those  NA, and infer the surfaces for these later 
BTS[!is.na(BTS$Distance) & BTS$Distance < 0,]$Distance <- NA

#now calculate surface
BTS$surface <- (BTS$Distance * BTS$BeamWidth)/1e+06

# summary of the surface var tells us the number of NAs, and whether there are negative values
summary(BTS$surface)
summary(BTS$HaulDur)
...
mesh1a   <- inla.mesh.2d(boundary = ConvHull,  max.edge=c(70))
```
...
ConvHull <- inla.nonconvex.hull(Loc, convex=-0.02, resolution=90)
mesh2a    <- inla.mesh.2d(boundary = ConvHull,  max.edge=c(70))
plot(mesh2a)
...
We do not plot the exploraory data aalaysis again.
...
set.seed(12345)
Sim <- inla.posterior.sample(n = 1, result = I1nb)

names(Sim[[1]])
head(Sim[[1]]$latent,15)
tail(Sim[[1]]$latent,15)
length(Sim[[1]]$latent)

# This gives an object with 6742 rows for this specific data set and model. The first xxx 
# rows are simulated values for eta = X * beta, where X is the matrix with covariates, and 
# the last 7 rows are simulated regression parameters. The middle part contains the simulated spatial field w.
# This is just one set of simulated values (due to n = 1 in the function above). 

# Here is some fancy code to graph the last 8 values
MyParams <- c("Intercept", "fYear2014", "fYear2015", "fYear2016","fYear2017", 
              "fSurveyNSIBTS", "surface")
rownum <- as.numeric(lapply(MyParams, 
              function(x) 
...
# note that these are the rownames of sim[[1]]$latent that start with "w:"
# This should be written more elegantly 

Betas <- Sim[[1]]$latent[as.numeric(rownum)]
wk    <- Sim[[1]]$latent[6239:(rownum[1]-1)]

Amatrix <- as.matrix(A1)

FixedPart   <- Xmatrix %*% Betas
SpatialPart <- Amatrix %*% wk
mu          <- exp(FixedPart + SpatialPart)
Ysim        <- rpois(n = nrow(LP), lambda = mu)
table(Ysim)

par(mfrow = c(1,1), mar = c(5,5,2,2), cex.lab = 1.5)
plot(table(Ysim),
     xlab = "Simulated endemic richness values",
     ylab = "Frequencies")
...
NSim <- 1000
SimData <- inla.posterior.sample(n = NSim, result = I3)
N  <- nrow(LP)
...
Xm <- as.matrix(X)
Am <- as.matrix(A2)
rownames(SimData[[1]]$latent)[2442:3633]
