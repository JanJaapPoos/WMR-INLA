...
Which vessels and years are present in BTS set? Note that there are some years in the BTS ISIS where the zero hauls are not added because no specimens were caught in that year (e.g. 2010). That shoul dbe fixed at ICES. Alternatively we could download the data with a more abundant species and make our own zero hauls.
```{r, eval=TRUE, echo=TRUE}
table(BTS$Ship, BTS$Year)
```
The BTS swept area estimate is rounded, and thus not very useful. We recalculate it: the distance and beam width are in m, so the surface is in m2, and divided by 1e+06 to get km2
```{r, eval=TRUE, echo=TRUE}
...
hist(full_cpue[full_cpue$NoPerHaul >0,]$LngtClass, breaks=100, xlab="Length (mm)", main="Observed lengths", ylim=c(0,2400))
abline(v=990, col="red")

lines(aggregate(NoPerHaul~LngtClass, data=full_cpue[full_cpue$NoPerHaul >0,], FUN="sum"))
...
We do not plot the exploratory data analayses again. As a reminder of the size of the dataset: the lenght aggregation has reduced it to `r  nrow(cpue_subset)` rows. This is the number of unique hauls in our data, which now spans years `r min(cpue_subset$Year)` to `r max(cpue_subset$Year)`.
...
cpue_subset <- cpue_subset[cpue_subset$Depth < 300,]
...
for (i in startyr:endyr){
    w.pm1km2 <- inla.mesh.project(wproj,w[w.st$w.group==which(i==Year)])
