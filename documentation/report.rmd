...
#########################################
# Spatial info
SpatField.w <- inla.spde2.result(inla = I1nb,
                                 name = "w",
                                 spde = spde,
                                 do.transfer = TRUE)
...
for (ii in startyr:2017){
  for (jj in 1:NSim){
    ...
    numbers[jj,as.character(ii)] <- sum(rres,na.rm=T)
    
    #use code below if only interested in fixed effects
    #res <- exp(lin)
    #numbers[jj,as.character(ii)] <-  res
  }
}

## Code in Alain Zuur course. Note that he has rpois
...
Number is the estimated number per km2 (minus the spatial effect)
...
matplot(t(qwtsNS), xlab="Years", ylab="Estimated Total Stock Weight (tonnes)")
```
