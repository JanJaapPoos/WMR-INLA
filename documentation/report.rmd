...
Because we now have combinations for all hauls and several length classes, the subset of data (since 2009) is still very big. It has`r  nrow(cpue_subset)` observations. In order to reduce the number of observations used in the INLA model, we take all non-zero observations but we subsample the zero observations. The remaining zero observations willneed to receive a higher weight in the final model. Note that this is not yet implemented.  
```{r, eval=F, echo=TRUE}
...
NExt, only the relevant variables are selected, and all hauls longer than 90 minutes removed from the data set.
```{r, eval=T, echo=T}
...
```{r, eval=TRUE, echo=TRUE, dpi=600}
plot(x=IBTSHH$HaulDur,y=IBTSHH$surface, pch=20)
linmod <- lm(surface~ -1 + HaulDur, data=IBTSHH)
summary(linmod)
abline(linmod, lty=2)
```

Indeed, there is a good relationship, where each minute of haul duration adds `coef(linmod)` km2 haul surface. This relationship is used to calculate the surface of hauls where only haul duration is known.   
```{r, eval=TRUE, echo=TRUE, dpi=600}
...
```{r, eval=TRUE, echo=TRUE, dpi=600}
plot(x=BTS$HaulDur,y=BTS$surface, pch=20)
...
We still have a lot of data (`r nrow(full_cpue)`), Because these are just here as example, we will again start from startyr `r startyr`.    
```{r, eval=TRUE, echo=TRUE}
cpue_subset <- full_cpue[full_cpue$Year >= startyr,]
```
...
We do not plot the exploratory data analayses again. As a reminder of the size of the dataset: the lenght aggregation has reduced it to `r  nrow(cpue_subset)` rows. This is the number of unique hauls in our data, which now spans years `r cpue_subset$Year`.
...
To go from number to weight we need alpha and beta from growth curve (for length in cm and resulting W is in kg )
alpha=0.156650
...
for (ii in startyr:2017){
  for (jj in 1:NSim){
            #intercept  +           year                         +  (1 times beta for surf)
    if (ii==startyr){
      lin <- linmat["Intercept",jj] +                                 1 * linmat["surface",jj]
    }else{
      lin <- linmat["Intercept",jj] + linmat[paste0("fYear",ii),jj] + 1 * linmat["surface",jj]
    }
